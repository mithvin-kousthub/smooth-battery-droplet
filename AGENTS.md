...
- Droplet id: `smooth-battery`. It is also `BatteryDroplet.id` in Swift and `id` in `droplet.json`; the three must agree or the loader refuses the bundle.
- Swift product: `Battery`, a dynamic library. The harness target is `BatteryHarness`.
- SDK checkout: `/Users/emvin/.gemini/antigravity/scratch/droppykit` (DroppyKit 1.8.1). Docs online: https://getdroppy.app/docs/droppykit
...
1. Edit `Sources/Battery/`. The manifest is `droplet.json`.
2. `droppykit build` writes `.build/Battery.droplet`, universal, linked against the
...
5. Put the bundle into Droppy Playground and confirm it loaded. Copy
   `.build/Battery.droplet` to
   `~/Library/Application Support/Droppy Playground/Droplets/smooth-battery/Battery.droplet`,
...
and `.cursor/mcp.json` (Cursor). Codex: `codex mcp add droppykit -- /Users/emvin/.gemini/antigravity/scratch/droppykit/Scripts/droppykit mcp`.
...
- **The principal class does nothing.** `BatteryPrincipal` is `@objc`, is named in the
...
- **No `main.swift`.** The harness entry is `@main` in
  `Sources/BatteryHarness/BatteryHarness.swift`, and a file named `main.swift` cannot
  coexist with `@main`.
...
- **Do not edit anything under `/Users/emvin/.gemini/antigravity/scratch/droppykit`.** That is the SDK checkout; fixes there go
...
- Guides, as Markdown: `/Users/emvin/.gemini/antigravity/scratch/droppykit/Sources/DroppyKit/Documentation.docc/`
  ...
  workflow in full.
- The surface protocols, one file each: `/Users/emvin/.gemini/antigravity/scratch/droppykit/Sources/DroppyKit/Capabilities/`
- The host services a droplet calls: `/Users/emvin/.gemini/antigravity/scratch/droppykit/Sources/DroppyKit/Services/DropletServices.swift`
  and `/Users/emvin/.gemini/antigravity/scratch/droppykit/Sources/DroppyKit/Core/DropletHost.swift`
- The manifest type, with every field documented: `/Users/emvin/.gemini/antigravity/scratch/droppykit/Sources/DroppyKit/Bundle/DropletManifest.swift`
- Design tokens and the settings components: `/Users/emvin/.gemini/antigravity/scratch/droppykit/Sources/DroppyKit/DesignSystem/`
- A complete droplet that uses every surface: `/Users/emvin/.gemini/antigravity/scratch/droppykit/Examples/WorldClock/`
- The compatibility promise and the version ledger: `/Users/emvin/.gemini/antigravity/scratch/droppykit/COMPATIBILITY.md`
...
```
Package.swift                     product Battery (dynamic) and BatteryHarness
droplet.json                      the manifest; Info.plist is generated from it
Sources/Battery/              the droplet
Sources/BatteryHarness/       the @main harness entry; never main.swift
Battery.icon/                 Icon Composer document, required
Assets/Creator.png                square creator avatar, at least 256px, required
.build/Battery.droplet        what droppykit build writes
